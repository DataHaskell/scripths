...
parseMarkdown' acc (line : rest) = case T.strip <$> T.stripPrefix fence line of
    ...
    Just lang -> let
            ...
            segments = [Prose prose | not (T.null prose)] ++ [CodeBlock lang (T.unlines codeLines) Nothing]
        in segments ++ parseMarkdown' [] rest'
...
renderSegment (CodeBlock lang code (Just output)) =
    if T.null (T.strip output) then fenceCodeSegment lang code else fenceCodeSegment lang code <> blockQuote output
