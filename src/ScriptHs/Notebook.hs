...
import ScriptHs.Parser (CabalMeta (..), Line (..), ScriptFile (..), parseScript, mergeMetas)
...
parseBlocks blocks = (metas, map (second scriptLines) sfs)
    where
        sfs = [(i, parseScript code) | (i, CodeBlock lang code _ ) <- blocks, isHaskell lang]
...
generatedMarkedScript = concatMap renderWithMarker
    where
        ...
        renderWithMarker (idx, ls) = ls ++ [Blank, HaskellLine ("putStrLn " <> T.pack (show (T.unpack (mkMarker idx)))), Blank]
...
splitByMarkers remaining (idx : rest) =
    let marker = mkMarker idx
     ...
     in if T.null after
            ...
            else (idx, T.strip before) : splitByMarkers (T.drop (T.length marker) after) rest
